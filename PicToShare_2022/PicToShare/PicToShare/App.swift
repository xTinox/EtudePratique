...
struct PTSApp: App {
    ...
    private let fsActionHandler : ActionHandler
    private let fsWatchHandler : WatchHandler
    ...
    init() {
        //print("ok")
        //FileManager.containerURL(FileManager)
        ...
        fsActionHandler = FSActionHandler(configurationManager, importationManager, calendarResource)
        
        //fsWatchHandler = FSWatchHandler(actionHandler: fsActionHandler, pathType: .containerPath(path: "pictoshare.watched"))

        fsWatchHandler = FSWatchHandler(actionHandler: fsActionHandler, pathType: .homedirPath(path: "Library/Mobile Documents/iCloud~pictosave/Documents".addingPercentEncoding(withAllowedCharacters: [])))
        
        for type in configurationManager.types {
            print(type)
        }
    }
